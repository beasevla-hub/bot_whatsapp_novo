...
def get_cor_status(status):
    # Qualquer status que comece com "SIM" é verde
    if status and str(status).startswith('SIM'):
...
def processar_aptos(thi_atestados, phas_atestados):
    ...
    # 3. regras de retorno com identificação da empresa apta
    if thi_is_apta and phas_is_apta:
        return 'SIM THI E PHAS'

    if thi_is_apta and not phas_is_apta:
        return 'SIM APTOS THI'

    if phas_is_apta and not thi_is_apta:
        return 'SIM APTOS PHAS'
